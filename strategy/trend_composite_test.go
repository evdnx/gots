...
// feedBars sends a slice of candles to the supplied TrendComposite instance.
func feedBarsTrendComposite(t *testing.T, tc *TrendComposite, bars []candle) {
	for _, b := range bars {
		tc.ProcessBar(b.high, b.low, b.close, b.volume)
	}
}

// buildTrendComposite creates a TrendComposite strategy wired to a mock
// executor and logger.  All oscillator thresholds are set to extreme
// values so the RSI/MFI checks never block a trade – the only gating
// factor is the HMA/AMDO/ATSO crossover logic.
func buildTrendComposite(t *testing.T) (*TrendComposite, *testutils.MockExecutor) {
	...
	tc := &TrendComposite{BaseStrategy: base, lastDir: 0}
	return tc, mockExec
}
...
-----------------------------------------------------------------------
An upward price ramp creates bullish crossovers for HMA, AMDO and
ATSO after the warm‑up period (≈10 bars).  Because the thresholds are
extreme, the RSI/MFI checks are always satisfied.
*/
func TestTrendComposite_LongEntry(t *testing.T) {
	...
	feedBarsTrendComposite(t, tc, bars)
...
-----------------------------------------------------------------------
A downward price ramp creates bearish crossovers for all three
indicators, leading to a short position.
*/
...
func TestTrendComposite_TrailingStop(t *testing.T) {
	...
	feedBarsTrendComposite(t, tc, up)
	...
	feedBarsTrendComposite(t, tc, []candle{{high, low, close, 1200}})
...
func TestTrendComposite_TakeProfit(t *testing.T) {
	...
	low := tpLevel - 0.5
	close := tpLevel + 0.1
	feedBarsTrendComposite(t, tc, []candle{{high, low, close, 1300}})
...
func TestTrendComposite_OppositeSideFlip(t *testing.T) {
	...
	feedBarsTrendComposite(t, tc, down)
